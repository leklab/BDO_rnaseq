...
run_deseq <- function(samples,group1,group2,comparison){
  ...
  pheatmap(mat, annotation_col=df, show_rownames = F, legend = F, annotation_names_col = F, filename = sprintf("/results/%s/%s_heatmap.png",comparison,comparison))
  
  print("PCA plot")
  vsd <- vst(dds, blind=FALSE)
  plotPCA(vsd, intgroup="condition") + theme_bw()
  ggsave(sprintf("/results/%s/%s_PCA.png",comparison,comparison), width = unit(7, 'in'), height  = unit(8, 'in'))
...
run_deseq(samples_BvsA,"Group_B_Th17","Group_A_CTL-org","GroupB_allvsGroupA")
run_deseq(samples_CvsA,"Group_C_IL17-org","Group_A_CTL-org","GroupCvsGroupA")
